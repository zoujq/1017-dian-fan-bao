<template>	
	<view class="content" >
			...
			<view class="dialog-content" >
				...
				<view class="set-food-mode" v-show="pop_show==2">
					...
					<view class="d-content">
						...
						<view class="yuyue">							
								<view>自动保温</view>							
								<switch @change="auto_bao_wen" ></switch>
						</view>
						<view class="yuyue">
		...
		<image class="idimg" src="../../static/dianfanbao/main-img.png" ></image>
		<view class="logo-qie" >
		...
		<view class="cup-state" >
			<view class="connect-state" >
				{{ble_state==0 ? '未连接' : ble_state==1 ? '连接中...' : work_mode==0 ? '空闲中' : work_mode==1 ? '烹饪中' : 
				work_mode==2 ? '预约中' : work_mode==3 ? '保温中' : '空闲中'}}
			</view>
			<view class="temp-state" :class="[ble_state == 2 && (work_mode==1 || work_mode==2) ? 'temp-state-show' : '']">
				<view class="temp-num">
					<view class="temp-num-v" >{{kai_fan_shi_jian}}</view>
					<view class="temp-num-c" >开饭</view>
				</view>
				<view class="temp-shuiwen" >{{dang_qian_shi_pu}}</view>
			...
			<view class="state-con">
				...
				<view class="stop-img" v-show="ble_state==2 && work_mode!=0" @click="stop_work">
					<image></image>
				</view>
	...
	export default { 
		...
		methods: {
			...
			on_received_data(js_arr){
				console.log(js_arr);
				if(js_arr[0]!=0xC8 || js_arr[1]!=2){
					return;
				}
				this.work_mode=js_arr[2];
				current_shi_pu=js_arr[3];
				var t1=new Date();
			...
			check_cup_state(){
				...
				connect_ble.send_to_device([0xC8,0x02]);
			},
			t1(){
				// uni.redirectTo({
				//     url: '../connect/connect'
